...
export default class SocialPushNotificationPlugin {
   ...
   */
  async init() {
    this.social_post_model = "kjzl6hvfrbw6c646f9as8ecl9ni6l5qh06hxnx1gbectvymjwjiz48dtlkadyrp";
    this.social_profile_model = "kjzl6hvfrbw6c9q2rtmzylcuvba782ck7co36wnoppn9x80qsq2oas01ou1zgwr";
    //this.createTestPost();
    return {
      HOOKS: {
  ...
  async getUsersToken(did) {
    let mentionedUser;
    let mentionedUserArr = global.indexingService.ceramic.orbisdb.select().from(this.social_profile_model).where({controller: did}).first()
    if(mentionedUserArr) {
