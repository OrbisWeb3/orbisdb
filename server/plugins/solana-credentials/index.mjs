...
export default class SolanaCredentials {
  ...
  async createModel() {
    ...
    let result = await global.indexingService.ceramic.orbisdb.ceramic.createModel(modelDefinition);
  ...
  async getTransactions(address) {
    ...
    const transactions = await this.connection.getSignaturesForAddress(new PublicKey(address));
    if(transactions) {
  ...
  async mintCredentials(req, res) {
    ...
    if(req.body) {
      ...
      res.json({ status: 200, address: address, credentials: credentials, streams });
    } else {
      res.json({ status: 300 });
    }
  };
  ...
  async upsertScore(did, xp) {
    ...
    try {
      stream = global.indexingService.ceramic.orbisdb
        .insert("kjzl6hvfrbw6c88zjg5ep2zjj09vag9e3xtp48bo4jce2mvkef9117e0wbwxqp3")
        .value({
          "receiver": did,
          "xp": xp
        })
  ...
  async upsertCredential(did, credential) {
    let stream;
    let model = "kjzl6hvfrbw6c8g3qm4nwr2ifl43mfy6ik2e44ikw27ryoos7i1ttde24tcm4s8";

    // Step 1: Retrieve credential from db (to check if already exists)
    let existingCredential;
    try {
      existingCredential = await global.indexingService.ceramic.orbisdb.select().from(model).where({"receiver": did, "slug": credential.slug}).context(this.context).orderBy(["indexed_at", "desc"]).limit(1).run();
    } catch(e) {
    ...
    if(existingCredential && existingCredential.rows.length > 0) {
      ...
      if(credential.count > cred.count) {
        ...
        stream = await global.indexingService.ceramic.orbisdb.update(cred.stream_id).replace(content).run();
      } else {
      ...
      try {
        stream = await global.indexingService.ceramic.orbisdb.insert(model).value(content)
          .context(this.context)
          .run();
