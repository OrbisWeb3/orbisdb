...
export default function ModelPicker({value, setValue}) {
    ...
    return(
      <div className={`flex w-full items-center flex-col border rounded-md mt-2 ${value ? "bg-emerald-50 border-emerald-200" : "bg-slate-50 border-slate-200 border-dashed" }`}>
        <StepsModel step={step} setStep={setStep} modelId={value} setModelId={setValue} />
      </div>
    )
}
...
const StepsModel = ({step, setStep, modelId, setModelId}) => {
    switch(step) {
        ...
        case 0:
            return(
              <div className="flex flex-col space-y-2 p-3">
                <div className="rounded-md bg-white border border-slate-200 p-3 cursor-pointer" onClick={() => setStep(1.0)}>
                  <p className="text-sm font-medium text-slate-900">Use existing model</p>
                  <p className="text-sm text-slate-600">Will use an existing model, you will only have to share the model ID you want to use.</p>
                </div>

                <div className="rounded-md bg-white border border-slate-200 hover:border-slate-300 p-3 cursor-pointer" onClick={() => setStep(1.1)}>
                  <p className="text-sm font-medium text-slate-900">Create model</p>
                  <p className="text-sm text-slate-600">Will create a new model on the Ceramic network based on your definition.</p>
                </div>
        ...
        case 1.0:
          return(
            <ModelSelect modelId={modelId} setModelId={setModelId} back={() => setStep(0)} />
          );
        case 1.1:
          return(
            <ModelBuilder setModelId={setModelId} back={() => setStep(0)} />
          );
        default:
            return null;
    }
...
export const ModelBuilder = ({ setModelId, back, showBack = true, createColor = "secondary"}) => {
    ...
    <>
      <div className="flex flex-col mt-2 pr-2">
        ...
        <div className="flex flex-col items-start mb-2">
            <div className="text-slate-600 font-medium text-sm mb-1">Model title:</div>
            <input
            type="text"
        ...
        <div className="text-slate-600 font-medium text-sm mb-1 mt-1">Model properties:</div>
      ...
      <div className="flex flex-col mt-3 justify-center items-center space-y-3 w-full mb-2">
        ...
        <Button title="Create model" type={createColor} onClick={(e) => createModel(e)} status={status} successTitle="Model created" />
      </div>
    </>
  )
}
...
function ModelFieldsInputGroups({inputGroups, setInputGroups}) {
  ...
  const BackButton = ({back}) => {
    return(
      <div className="flex cursor-pointer space-x-1.5 hover:underline items-center mb-2" onClick={back}>
          ...
          <span className="flex text-slate-6900 text-sm ml-1 font-medium">Back</span>
      </div>
    )
  }