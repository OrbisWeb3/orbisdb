  ...
  return(
    <Modal hide={() => setAddModalVis(false)}>
      <div className="flex flex-col justify-center">
        ...
        <AddModelSteps step={step} setStep={setStep} setAddModalVis={setAddModalVis} />
      </div>
    </Modal>
  )
}
...
const AddModelSteps = ({step, setStep, setAddModalVis}) => {
  ...
  switch (step) {
    ...
    case 2:
      return(
        <>
          <p className="text-base text-slate-500 mb-2">Model name: <span className="font-medium text-slate-900">{modelDetails.name ? modelDetails.name : modelDetails.title}</span></p>
    ...
    return(
      <>
        <p className="text-base text-slate-900 mb-3 text-center">We will now initialize the indexing of this model which is going to create a new table in your database with those fields.</p>
        <div className="mb-3 justify-center flex">
          {modelDetails.schema ?
            <SchemaDetails content={modelDetails} properties={modelDetails.schema.properties} />
          :
            <p className="rounded-md p-2 bg-slate-50 text-center w-full">This stream is not a model.</p>
          }
        </div>
        <Button type="primary" onClick={() => startIndexing()} status={status} title="Start indexing" />
      </>
    );
    default:
...
const SchemaDetails = ({content, properties}) => {
  if(properties) {
    ...
    return <p className="text-gray-500 text-md">We couldn't find any property for this schema.</p>
  }
}
...
const ValueDetails = ({val}) => {
  if(Array.isArray(val)) {
    return val.map((type, key) => {
      if(type) {
        return(
          <>:<span className="rounded-md px-2 py-1 bg-slate-100 text-gray-600 mr-1.5" key={key}>{type}</span></>
        )
      } else {
        return null;
      }
    ...
    if(val) {
      return <>:<span className="rounded-md px-2 py-1 bg-slate-100 text-gray-600">{val}</span></>;
    } else {
      return null;
    }
...
const SchemaType = ({properties, isParent}) => {
  return properties.map(([key, value]) => (
    <>
      <li key={key}><span className={isParent ? "text-gray-900 font-mono" : "text-gray-500 font-mono"}>{key}</span> <ValueDetails val={value.type} /></li>
      {(value?.type?.includes('object') && value.properties) &&
        <div className="pl-4 mt-2 border-l border-gray-200 space-y-2">
          <SchemaType properties={Object.entries(value.properties)} isParent={false} />
        </div>
      }
    </>
  ))
